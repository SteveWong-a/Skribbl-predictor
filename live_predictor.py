    ...
    while True:
        ...
        if current_time - last_grab_time >= TIMER_INTERVAL:
            ...
            try:
                ...
                # Check if we got a manually typed hint
                if current_hint:
                    hint_pattern = current_hint
                else:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_BACKSPACE:
                    ...
                    current_hint += event.unicode.lower()
                
                # Automatically format: replace double spaces with single space
                current_hint = re.sub(r'[ \t]{2,}', ' ', current_hint)
                last_grab_time = 0 # force update
        ...
        display_hint = str(current_hint) if current_hint else ("_" * target_length)
        # Show a cursor effect for typing
        cursor = "|" if time.time() % 1 > 0.5 else ""
        screen.blit(font.render(f"Type Hint: {display_hint}{cursor}", True, (0,0,0)), (20, 120))
